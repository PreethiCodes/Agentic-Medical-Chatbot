...
const Chatbot = () => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setMessages((prev) => [
          ...prev,
          { id: Date.now() + 1, text: `Error: ${data.error}`, isUser: false },
        ]);
        setStatus('Error');
      }
    } catch (err) {
      const errorMessage =
        err.response?.data?.error || err.message || 'Connection error. Please check if the backend is running on port 5000.';
      setMessages((prev) => [
        ...prev,
        { id: Date.now() + 1, text: errorMessage, isUser: false },
      ]);
      setStatus('Error');
    } finally {
      setIsLoading(false);
  ...
  const renderMessageContent = (m) => {
    ...
    try {
      ...
      const riskColor =
        riskVal > 70 ? 'text-red-400' : riskVal > 30 ? 'text-yellow-400' : 'text-green-400';
      const riskBg =
        riskVal > 70 ? 'bg-red-500/20' : riskVal > 30 ? 'bg-yellow-500/20' : 'bg-green-500/20';
              ...
              <div className="space-y-2 bg-green-500/5 p-3 rounded-xl border border-green-500/10">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {data.hospitals.map((hospital, idx) => (
                    <div
                      key={idx}
                      className="flex flex-col p-2 bg-white/5 rounded-lg border border-white/5 hover:border-green-500/20 transition-colors"
                    >
  ...
  return (
    <div className="pt-24 h-screen flex flex-col max-w-4xl mx-auto px-4">
      <div className="flex items-center justify-between mb-4 glass-card px-6 py-4 rounded-2xl">
        ...
        <button
          ...
          className={`p-3 rounded-xl transition-all ${
            voiceEnabled ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-800 text-gray-500'
          }`}
        >
      ...
      >
        <AnimatePresence initial={false}>
          ...
          {messages.map((m) => (
            ...
            >
              <div
                className={`flex gap-3 max-w-[85%] ${m.isUser ? 'flex-row-reverse' : 'flex-row'}`}
              >
                <div
                ...
                <div
                  className={`p-4 rounded-2xl glass-card ${
                    m.isUser ? 'bg-yellow-500/5' : 'bg-purple-500/5'
                  }`}
                >
      ...
      <form onSubmit={handleSend} className="mb-8 relative">
