...
const ReportAgent = () => {
  ...
  const handleFiles = (newFiles) => {
    ...
    if (filtered.length !== newFiles.length) {
      ...
      alert(
        `${rejectedCount} file(s) were rejected. Supported formats: PDF, PNG, JPG, JPEG, BMP, TIFF, DICOM`
      );
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (successCount === 0) {
        ...
        setError('All files failed to process. Please check if the backend is running.');
      ...
      setUploadStatus('error');
      setError(err.message || 'Network error. Please ensure the backend is running on port 5000.');
    } finally {
    ...
    <div className="pt-24 min-h-[calc(100vh-80px)] max-w-5xl mx-auto px-4">
      <div className="text-center mb-12">
        ...
        <p className="text-gray-400 max-w-xl mx-auto">
          Upload your medical reports, laboratory results, or prescriptions. Our AI will analyze
          them and provide detailed insights.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <motion.div
            ...
            className={`relative glass-card rounded-3xl p-12 border-2 border-dashed transition-all duration-300 flex flex-col items-center justify-center text-center ${
              isDragging ? 'border-yellow-500 bg-yellow-500/5' : 'border-white/10 hover:border-white/20'
            }`}
          ...
          >
            ...
            <div
              className={`w-20 h-20 rounded-2xl bg-yellow-500/10 flex items-center justify-center mb-6 border border-yellow-500/20 ${
            ...
            <p className="text-gray-500 mb-6">
              Support for PDF, PNG, JPG, JPEG, BMP, TIFF, DICOM (Max 10MB per file)
            </p>

            <div className="flex flex-wrap gap-2 justify-center">
              {['PDF', 'PNG', 'JPG/JPEG', 'BMP', 'TIFF', 'DICOM'].map((fmt) => (
                <div
                  key={fmt}
                  className="flex items-center gap-2 text-xs text-gray-400 bg-white/5 px-3 py-1 rounded-full border border-white/5"
                >
                  {fmt.includes('PDF') || fmt.includes('DICOM') ? (
                    <FileText size={12} />
                  ) : (
                    <ImageIcon size={12} />
                  )}
                ...
                {files.map((file, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-medium text-white max-w-[200px] md:max-w-md truncate">
                          {file.name}
                        </p>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card rounded-3xl p-6 border border-white/10 bg-purple-900/10">
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              Your documents are processed securely. No personal data is stored beyond the analysis
              session.
            </p>
          </div>
                ...
                <div className="space-y-4">
                  ...
                  {result.audioUrl && (
                    <div className="pt-4 border-t border-white/10 flex items-center gap-4">
                      <button
                        onClick={() =>
                          isPlaying ? stopAudio() : playAudio(result.audioUrl)
                        }
